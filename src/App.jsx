...
const ADMIN_WHATSAPP   = '5491100000000'           // Tu número sin + (ej: 5491155667788)
const MERCADOPAGO_LINK = 'https://mpago.la/XXXXXX' // Tu link de cobro de Mercado Pago
const ADMIN_LOCAL_PASS = 'oasis2024'               // Clave de acceso al panel admin
...
// ─── LOGO PLACEHOLDER (reemplazá con tu imagen real) ─────────────────────────
...
function FloatingChat() {
  ...
  const fabStyle = { position: 'fixed', bottom: 24, right: 24, zIndex: 50, width: 64, height: 64, borderRadius: '50%', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'float 3s ease-in-out infinite' }

  if (!user) return (
    <a href={`https://wa.me/${ADMIN_WHATSAPP}?text=Hola%20Oasis!`} target="_blank" rel="noopener noreferrer"
      style={{ ...fabStyle, background: '#25D366', boxShadow: '0 0 25px rgba(37,211,102,0.5)', color: '#fff', textDecoration: 'none' }}>
      <Ico.WA />
    </a>
  )
  ...
  return (
    <>
      <button onClick={() => setOpen(!open)} style={{ ...fabStyle, background: 'linear-gradient(135deg,#7c3aed,#a855f7)', boxShadow: '0 0 25px rgba(168,85,247,0.6)', color: '#fff' }}>
        {open ? <Ico.Close /> : <Ico.WA />}
      </button>

      {open && (
        <div style={{ position: 'fixed', bottom: 100, right: 24, zIndex: 50, width: 320, borderRadius: 24, overflow: 'hidden', background: '#0a0a1a', border: '1px solid rgba(168,85,247,0.4)', boxShadow: '0 0 40px rgba(168,85,247,0.25)' }}>
          <div style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12, background: 'linear-gradient(135deg,#7c3aed,#a855f7)' }}>
            <OasisLogo size={36} />
            <div><div style={{ fontFamily: 'Orbitron,monospace', fontWeight: 700, color: '#fff', fontSize: 14 }}>OASIS</div><div style={{ fontSize: 12, color: '#ddd6fe' }}>Soporte en línea ✓</div></div>
          </div>
          <div style={{ height: 220, overflowY: 'auto', padding: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
            {msgs.length === 0 && <div style={{ color: '#a855f7', fontSize: 14, textAlign: 'center', marginTop: 32 }}>👋 ¡Hola! Escribinos tu consulta.</div>}
            {msgs.map(m => (
              <div key={m.id} style={{ display: 'flex', justifyContent: m.sender === 'user' ? 'flex-end' : 'flex-start' }}>
                <div style={{ maxWidth: '80%', padding: '8px 12px', borderRadius: 16, fontSize: 13, ...(m.sender === 'user' ? { background: 'linear-gradient(135deg,#7c3aed,#a855f7)', color: '#fff', borderBottomRightRadius: 4 } : { background: '#1f1f35', color: '#e2e8f0', borderBottomLeftRadius: 4 }) }}>
                  {m.content}
                </div>
              </div>
            ))}
...
function CartModal({ setPage }) {
  ...
  const placeOrder = async () => {
    ...
    const { error } = await supabase.from('orders').insert({ user_id: user.id, order_code: code, items: orderItems, total, payment_method: method, whatsapp: wa, notes, status: 'received' })
    if (!error) {
      // Descontar stock
      for (const item of items) {
        if (item.stock > 0) await supabase.from('products').update({ stock: Math.max(0, item.stock - item.qty) }).eq('id', item.id)
      }
      setOrderCode(code)
      clearCart()
      setStep('done')
      if (method === 'cash') {
        const lines = orderItems.map(i => `- ${i.name} x${i.qty}: $${(i.price * i.qty).toLocaleString('es-AR')}`).join('\n')
        const msg = encodeURIComponent(`🛒 *Nuevo Pedido OASIS*\nCódigo: *${code}*\n\n${lines}\n\n💰 Total: *$${total.toLocaleString('es-AR')}*\nPago: Efectivo${wa ? `\n📱 ${wa}` : ''}${notes ? `\n📝 ${notes}` : ''}`)
  ...
  const overlay = { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(6px)', zIndex: 60 }
        ...
        <div style={{ padding: 24 }}>
          ...
          {step === 'done' ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16, animation: 'fadeIn 0.4s' }}>
              ...
              <input value={wa} onChange={e => setWa(e.target.value)} placeholder="Tu WhatsApp (opcional)" />
...
function HomePage() {
  ...
  const filtered = activeTab === 'all' ? products : products.filter(p => p.category_id === activeTab)
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 16px' }}>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', paddingTop: 80, color: '#a855f7' }}>
          ...
          <p style={{ fontSize: 14, color: '#4b5563', marginTop: 8 }}>Volvé pronto o consultanos por WhatsApp</p>
        </div>
      ) : (
...
function LoginPage({ setPage }) {
  ...
  const handleLogin = async () => {
    ...
    const { error } = await supabase.auth.signInWithOtp({ email: email.trim(), options: { data: { full_name: name.trim() }, emailRedirectTo: window.location.origin } })
        ...
        <div className="card-neon" style={{ borderRadius: 28, padding: 32 }}>
          {sent ? (
            <div style={{ textAlign: 'center', animation: 'fadeIn 0.4s' }}>
              ...
              <h3 style={{ color: '#fff', fontWeight: 700, fontSize: 22, marginBottom: 10 }}>¡Revisá tu correo!</h3>
              <p style={{ color: '#c084fc', fontSize: 15 }}>Te enviamos un link mágico a <strong>{email}</strong>. Hacé click ahí para entrar sin contraseña.</p>
              <button onClick={() => setSent(false)} style={{ marginTop: 20, background: 'none', border: 'none', color: '#a855f7', cursor: 'pointer', textDecoration: 'underline' }}>Usar otro email</button>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                <label style={{ color: '#c084fc', fontSize: 13, fontWeight: 600, display: 'block', marginBottom: 6 }}>Tu nombre</label>
                <input value={name} onChange={e => setName(e.target.value)} placeholder="¿Cómo te llamás?" />
              </div>
              <div>
                <label style={{ color: '#c084fc', fontSize: 13, fontWeight: 600, display: 'block', marginBottom: 6 }}>Email</label>
                <input value={email} onChange={e => setEmail(e.target.value)} type="email" placeholder="tu@email.com" onKeyDown={e => e.key === 'Enter' && handleLogin()} />
              </div>
              {error && <p style={{ color: '#f87171', fontSize: 13 }}>{error}</p>}
              <Btn style={{ width: '100%', marginTop: 4 }} size="lg" onClick={handleLogin} disabled={loading}>
                {loading ? 'Enviando...' : 'Entrar con Magic Link ✨'}
              </Btn>
              <p style={{ color: '#4b5563', fontSize: 12, textAlign: 'center' }}>Recibís un link en tu email. Sin contraseñas.</p>
            </div>
          )}
        </div>
        <button onClick={() => setPage('home')} style={{ display: 'block', margin: '20px auto 0', background: 'none', border: 'none', cursor: 'pointer', color: '#7c3aed' }}>← Volver a la tienda</button>
      </div>
    </div>
  )
}
...
function AccountPage() {
  ...
  const statuses = { received: { label: 'Recibido', color: '#60a5fa', bg: 'rgba(59,130,246,0.15)', icon: '📥' }, preparing: { label: 'En Preparación', color: '#fbbf24', bg: 'rgba(251,191,36,0.15)', icon: '⚙️' }, ready: { label: 'Listo para Retirar', color: '#4ade80', bg: 'rgba(74,222,128,0.15)', icon: '✅' } }
...
function AdminPage({ setPage }) {
  ...
  const tabs = [{ id: 'orders', label: '📦 Pedidos' }, { id: 'categories', label: '🏷️ Categorías' }, { id: 'products', label: '🛒 Productos' }, { id: 'banner', label: '🎯 Banner' }, { id: 'broadcast', label: '📢 Difusión' }, { id: 'chat', label: '💬 Chat' }]
...
function AdminOrders() {
  ...
  const statusMap = { received: { label: 'Recibido', color: '#60a5fa', icon: '📥' }, preparing: { label: 'En Preparación', color: '#fbbf24', icon: '⚙️' }, ready: { label: 'Listo ✅', color: '#4ade80', icon: '✅' } }
  ...
  return (
    <div>
      ...
      {orders.length === 0 ? <div style={{ textAlign: 'center', color: '#a855f7', padding: 48 }}><div style={{ fontSize: 40 }}>📭</div><p style={{ marginTop: 12 }}>No hay pedidos todavía</p></div> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {orders.map(order => {
              ...
              <div key={order.id} className="card-neon" style={{ borderRadius: 20, padding: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
                  <div>
                    ...
                    {order.whatsapp && <a href={`https://wa.me/${order.whatsapp.replace(/\D/g,'')}`} target="_blank" rel="noopener noreferrer" style={{ color: '#4ade80', fontSize: 13, display: 'flex', alignItems: 'center', gap: 4, marginTop: 4 }}>📱 {order.whatsapp}</a>}
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    {['received','preparing','ready'].map(s => (
                      <button key={s} onClick={() => setStatus(order.id, s)}
                        style={{ padding: '6px 14px', borderRadius: 50, fontWeight: 600, fontSize: 12, fontFamily: "'Exo 2',sans-serif", cursor: 'pointer', border: order.status === s ? 'none' : '1px solid rgba(168,85,247,0.4)', transition: 'all 0.2s', ...(order.status === s ? { background: 'linear-gradient(135deg,#7c3aed,#a855f7)', color: '#fff' } : { background: 'transparent', color: '#a855f7' }) }}>
                        {statusMap[s].icon}
                      </button>
                    ))}
                  </div>
      ...
      {categories.map(cat => (
        <div key={cat.id} className="card-neon" style={{ borderRadius: 18, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ color: '#6b7280', fontSize: 12 }}>slug: {cat.slug}</div>
          </div>
...
function AdminProducts() {
  ...
  const save = async () => {
    ...
    await supabase.from('products').insert({ name: form.name.trim(), description: form.description.trim(), price: parseFloat(form.price), stock: parseInt(form.stock) || -1, category_id: form.category_id, image_url, active: true })
...
function AdminBroadcast() {
  ...
  const broadcast = async () => {
    ...
    if (profiles?.length) await supabase.from('messages').insert(profiles.map(p => ({ user_id: p.id, sender: 'admin', content: `📢 ${msg.trim()}` })))
  ...
  return (
    <div className="card-neon" style={{ borderRadius: 20, padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <Btn style={{ width: '100%' }} size="lg" onClick={broadcast} disabled={sending || !msg.trim()}>{sending ? 'Enviando...' : '📢 Enviar a Todos'}</Btn>
    </div>
  )
}
...
function AdminChat() {
  ...
  useEffect(() => {
    supabase.from('messages').select('user_id, content, created_at, sender, profiles(full_name)').order('created_at', { ascending: false })
      .then(({ data }) => {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: selected ? '200px 1fr' : '1fr', gap: 14, minHeight: 400 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {convs.length === 0 ? <p style={{ color: '#6b7280', textAlign: 'center', paddingTop: 32 }}>Sin mensajes</p> : convs.map(c => (
          <button key={c.user_id} onClick={() => loadMsgs(c.user_id)}
            style={{ textAlign: 'left', padding: 12, borderRadius: 14, border: 'none', cursor: 'pointer', transition: 'all 0.2s', fontFamily: "'Exo 2',sans-serif", ...(selected === c.user_id ? { background: 'linear-gradient(135deg,#7c3aed,#a855f7)', color: '#fff' } : { background: '#0f0f1a', color: '#c084fc', border: '1px solid rgba(168,85,247,0.2)' }) }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
            {msgs.map(m => (
              <div key={m.id} style={{ display: 'flex', justifyContent: m.sender === 'admin' ? 'flex-end' : 'flex-start' }}>
                <div style={{ maxWidth: '75%', padding: '8px 14px', borderRadius: 16, fontSize: 14, ...(m.sender === 'admin' ? { background: 'linear-gradient(135deg,#7c3aed,#a855f7)', color: '#fff' } : { background: '#1f1f35', color: '#e2e8f0' }) }}>{m.content}</div>
              </div>
            ))}
          </div>
          <div style={{ padding: 12, display: 'flex', gap: 8, borderTop: '1px solid rgba(168,85,247,0.2)' }}>
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    // Handle magic link redirect (Supabase appends tokens to URL)
